...
## Первый релиз
<img src="https://habrastorage.org/webt/q9/hc/vm/q9hcvmkzwn-zrvfvdr8qzlsbd0w.png" />
...
Команде разработки выдали логи, она принилась за исправление, и параллельно начала обдумывать как бы улучшить 
code review и тестирование проекта, что бы более не сталкиваться с подобными проблемами. 

Но это уже совсем другая история, а нам пора подводить итоги.

## Итоги

TBD - отличная модель ветвления, которая наконец-то поможет вам избавится от кашмара слияния веток, от беспорядочного создания 
веток, позволит упростить вам code review, процессы тестирования и внедрения. 
Spring boot конечно не является необходимым фактором для применения TBD, но надеюсь я смог показать на сколько он упрощает
реализацию основных его приемов.  
